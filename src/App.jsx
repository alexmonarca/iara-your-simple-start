...
import AdminPricingOverridePanel from './components/admin/AdminPricingOverridePanel.jsx';
...
// Lê o "total contratado" persistido no banco (fail-safe). Se existir override no admin, ele manda.
const getContractedTotalFromSubscription = (sub) => {
  const addons = sub?.addons;
  if (!addons || typeof addons !== 'object') return null;
  const override = Number(addons.override_total);
  if (Number.isFinite(override) && override > 0) return override;
  const total = Number(addons.total);
...
function Dashboard({ session }) {
  ...
  useEffect(() => { 
      const fetchData = async () => { 
          ...
          try { 
              ...
              // Importante: traz todos os campos para permitir ler subscriptions.addons.total / override_total
              let sub = await supabaseClient.from('subscriptions').select('*').eq('user_id', userId).maybeSingle();
              if (!sub.data) {
                await supabaseClient.from('subscriptions').insert({ user_id: userId, plan_type: 'trial_7_days', status: 'active', addons: {} });
                sub = { data: { plan_type: 'trial_7_days', status: 'active', addons: {} } };
              }
  ...
  // Total contratado persistido (para evitar inconsistências no resumo)
  const contractedTotal = getContractedTotalFromSubscription(subscriptionInfo);
  
  const isActiveSubscriber = isSubscriptionActive(subscriptionInfo.status) && subscriptionInfo.plan_type !== 'trial_7_days';
  const upgradeBase = isActiveSubscriber ? (contractedTotal ?? 250) : 0;
  const upgradeValue = isActiveSubscriber ? Math.max(0, totalPrice - upgradeBase) : totalPrice;
  ...
  const displayPlanName = isSubscriptionActive(subscriptionInfo.status) && subscriptionInfo.plan_type !== 'trial_7_days' 
      ? getPlanNameByPrice(contractedTotal ?? totalPrice, subscriptionInfo.plan_type)
      : 'Trial Grátis';

  const buildAddonsSnapshot = ({ newContractedTotal }) => {
    const items = [];

    if (gymData?.branches?.length > 0) {
      items.push({ code: 'branches', qty: gymData.branches.length, unit_price: 150, price: gymData.branches.length * 150 });
    }
    if (extraChannels > 0) items.push({ code: 'extra_channels', qty: extraChannels, unit_price: 50, price: extraChannels * 50 });
    if (gymData?.extra_users_count > 0) items.push({ code: 'extra_users', qty: gymData.extra_users_count, unit_price: 50, price: gymData.extra_users_count * 50 });
    if (gymData?.omnichannel) items.push({ code: 'omnichannel', qty: 1, unit_price: 150, price: 150 });
    if (gymData?.integrate_agenda) items.push({ code: 'integrate_agenda', qty: 1, unit_price: 50, price: 50 });
    if (gymData?.recognize_payments) items.push({ code: 'recognize_payments', qty: 1, unit_price: 50, price: 50 });
    if (gymData?.mass_sender_active) items.push({ code: 'mass_sender', qty: 1, unit_price: 150, price: 150 });
    if (gymData?.use_official_api_coexistencia) items.push({ code: 'official_api_coexistencia', qty: 1, unit_price: 50, price: 50 });
    if (gymData?.use_official_api_somente) items.push({ code: 'official_api_somente', qty: 1, unit_price: 150, price: 150 });
    if (gymData?.ia_gestor_midias) items.push({ code: 'ia_gestor_midias', qty: 1, unit_price: 250, price: 250 });

    return {
      // Snapshot do contrato (base sempre 250; desconto onboarding pode ser 0..100)
      base_price: 250,
      onboarding_discount: Number(onboardingDiscount) || 0,
      coupon_code: appliedCoupon?.code || null,
      coupon_discount: appliedCoupon?.amount || 0,
      items,
      total: Number(newContractedTotal) || 0,
      updated_at: new Date().toISOString(),
      // Mantém override se existir
      override_total: subscriptionInfo?.addons?.override_total ?? null,
    };
  };

  const handleSubscribe = async () => {
    if (finalCheckoutValue > 1250) { setIsHighTicketOpen(true); return; }
    if (isActiveSubscriber && upgradeValue <= 0) { handleSave(); alert("Plano atualizado com sucesso (sem custo adicional)."); return; }

    // Antes de abrir o checkout, persistimos o total contratado no banco (fail-safe do painel)
    // - new: total = totalPrice
    // - upgrade: total = (contratado atual) + upgradeValue
    try {
      const current = contractedTotal ?? 250;
      const newContractedTotal = isActiveSubscriber ? (current + upgradeValue) : totalPrice;
      const addonsSnapshot = buildAddonsSnapshot({ newContractedTotal });
      ...
      await supabaseClient
        ...
        .upsert(
          {
            user_id: userId,
            plan_type: subscriptionInfo?.plan_type ?? 'trial_7_days',
      ...
      // Atualiza em memória para o resumo já refletir
      setSubscriptionInfo((prev) => ({ ...(prev || {}), addons: addonsSnapshot }));
    } catch (e) {
      console.error('Falha ao persistir total contratado:', e);
      // Não bloqueia checkout: só perde o fail-safe.
    }
    ...
    try {
      const response = await fetch(WEBHOOK_SALES_URL, { method: 'POST', mode: 'cors', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
      if (!response.ok) throw new Error("Resposta inválida");
      const responseText = await response.text();
      setTimeout(() => { setCheckoutHtml(responseText); setCheckoutProcessing(false); }, 1500);
    } catch (error) {
          ...
          <div className="max-w-5xl mx-auto animate-in fade-in duration-500">
            ...
            {isSuperAdmin ? (
              <div className="space-y-8">
                ...
                <AdminPricingOverridePanel supabaseClient={supabaseClient} adminEmail={session.user.email} />
              </div>
            ) : (
